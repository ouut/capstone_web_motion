...
function validateWsInputs(): string | null {
  ...
  if (!h || !RE_HOST.test(h)) return 'Host: ws://localhost:8080 or wss://example.com';
  return null;
}
...
function packLandmarks(pts: number[]): Uint8Array {
  const buf = new ArrayBuffer(33);
  const dv = new DataView(buf);
  dv.setUint8(0, getMode() === 'hand' ? 0x01 : 0x02);
  for (let i = 0; i < 8; i++) dv.setFloat32(1 + i*4, pts[i] || 0, true);
  return new Uint8Array(buf);
}

function extractLandmarks(result: any): number[] {
  // 前置摄像头：x 翻转 (canvas 由 CSS rotateY 处理，这里只翻转 WebSocket 数据)
  const fx = (v: number) => 1 - v;
  if (getMode() === 'hand') {
...
let tracked: Map<string, TrackCallback | null> = new Map();
let throttle = 10;
let useCsv = false;
...
function colorDelta(cur: number, prev: number): string {
  ...
  if (d > 0.03) return '\x1b[38;5;196m';
  if (d > 0.01) return '\x1b[38;5;214m';
  if (d > 0.003) return '\x1b[38;5;228m';
...
const hl: HL = {
  ...
  status() {
    const m = getMode(); const names = [...tracked.keys()].filter(k => k !== '*');
...
btn.addEventListener('click', async () => {
  ...
  if (getMode() === 'hand') initParams.numHands = 2;
  else { initParams.numPoses = 1; initParams.outputSegmentationMasks = false; }
...
function loop() {
  ...
  if (now - fpsLastTime >= 1000) { fpsEl.textContent = `FPS: ${fpsFrames}`; fpsFrames = 0; fpsLastTime = now; }
